	...
	var af = (function(window) {
			...
			*/
			...
			serialize: function() {
				...
				var serializeFn = function(elem) {
					...
					if(elem.nodeName.toLowerCase() !== "fieldset" && type !== "submit" && type !== "reset" && type !== "button" && ((type !== "radio" && type !== "checkbox") || elem.checked)) {
		...
		*/
		$.ajax = function(opts) {
			...
			//          if(opts.url==LastAjaxUrl&&!settings.retryTimes){
			//              return;
			//          }
			if(!settings.retryTimes && opts.url == LastAjaxOpts.url && opts.data === LastAjaxOpts.data) { //data如果为对象则不能用==比较，always false
				return;
			}
			//ymabc
			if(settings.retryTimes) {
				settings.retryTimes = settings.retryTimes - 1; //每重试一次，次数便减一 
			} else {
				//              LastAjaxUrl=opts.url;//保留第一次url，后边如遇同样url且无retryTimes，则属不正常情况
				LastAjaxOpts = opts; //将opts保存起来，做下次比较
				settings.retryTimes = RETRYTIMES;
			}
			try {
				if(!settings.url)
					...
					if(xhr.readyState === 4) {
						...
						if((xhr.status >= 200 && xhr.status < 300) || xhr.status === 0 && protocol === "file:") {
							...
							if(xhr.status === 0 && result.length === 0)
								error = true;
							if(error) {
								if(settings.retryTimes > 0 && settings.dataType != "application/json") { //失败了便重试，直到重试次数为0（排除json是为了排除请求本地help下的文件的多次请求）
									$.ajax(opts);
								} else {
							...
							if(settings.retryTimes > 0 && settings.dataType != "application/json") { //失败了便重试，直到重试次数为0
								$.ajax(opts);
							} else {
								error = true;
				...
				plus.runtime.getProperty(plus.runtime.appid, function(wgtinfo) {
					//header params：contentType、token、ver、portType	
					if(settings.contentType) settings.headers["Content-Type"] = settings.contentType;
					settings.headers["x-auth-token"] = common.getlocalStorageItemStr("token");
					...
					if(settings.timeout > RETRYOUTTIME) { //在这儿设置超时时限
						abortTimeout = setTimeout(function() {
							if(settings.retryTimes > 0 && settings.dataType != "application/json") { //失败了便重试，直到重试次数为0
								$.ajax(opts);
							} else {
		...
		* @title $.post(url,[data],success,[dataType],contentType)
		*/
		$.post = function(url, data, success, error, dataType, contentType) {
			if($.isFunction(data)) {
				success = data;
				data = {};
			}
		...
		/**
		* Shorthand call to an Ajax PUT request
		    ```
		    $.put("mypage.php",{bar:"bar"},function(data){});
		    ```
		* @param {string} url URL to hit
		* @param {Object|Function|undefined} data Data to pass in or the success callback
		* @param {Function=} success [optional] Success Callback when data is given
		...
		*/
		...
		/**
		* Shorthand call to an Ajax DELETE request
		    ```
		    /api/release/student/{id}
		    $.delete("/api/release/student",{1},function(data){});
		    ```
		* @param {string} url URL to hit
		* @param {id} data Data to pass in or the success callback
		* @param {Function=} success [optional] Success Callback when data is given
		* @title $.delete(url,[data],success)
		*/
		...
		*/
		$.getJSON = function(url, data, success, error, contentType) {
			if(typeof(data) === "function") {
				error = success;
				success = data;
				data = {};
			}
			if(url.indexOf("/mockupsvr/") > 1) { //used to development test only,mockup server return 
				if(url.indexOf("?") > 0) {
		...
		*/
		...
		function detectUA($, userAgent) {
			...
			//          $.feat.nativeTouchScroll = true; //ymkadun-2016/1/6
