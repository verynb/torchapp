...
(function() {
	...
	var common = {
		...
		loadPage: function(obj, readOnly, noback, isback, forceflash) {
			...
			if($.ui.activeDiv.id != undefined && ($.ui.activeDiv.id != tkey || forceflash)) { //当前页面不能执行刷新操作
		...
		autoUpgrate: function(data, callback) {
			...
			plus.runtime.getProperty(appid, function(wgtinfo) {
				...
				//				data.url = "http://www.60living.com/testFile/org.youyougongyi.torch.wgt";
				if((data.isForce == true && currentVer < data.version) || data.isForce == false && currentVer != data.version) {
					$.ui.popup({
						title: "有新版本",
						...
						doneCallback: function() {
							...
							var fileType = strurl.substr(strurl.length - 3, 3);
							if(fileType != 'apk' && fileType != 'ipa') {
								that.upgrade(data);
							} else {
								if($.os.ios) {
									common.toast("展示没有下载地址");
		...
		/*原生日期 2014-01-01*/
		pickDateYMD: function(obj, callback) {
			var that = this;
			...
			try {
				if(plus) {
					...
					var setMinDate = ($(obj).attr("data-minDate")) ? $(obj).attr("data-minDate") : minDate.getFullYear();
					minDate.setFullYear(setMinDate, 0, 1); //设置日期最小值
					var maxDate = new Date();
					maxDate.setFullYear(maxDate.getFullYear(), maxDate.getMonth(), maxDate.getDate()); //设置日期最大值
					plus.nativeUI.pickDate(function(e) {
						var d = e.date; //被选中日期
						var choose_y = d.getFullYear();
						var choose_mo = d.getMonth() + 1 < 10 ? ("0" + (d.getMonth() + 1)) : (d.getMonth() + 1);
						var choose_d = d.getDate() < 10 ? ("0" + d.getDate()) : d.getDate();
						var choose_h = d.getHours() < 10 ? ("0" + d.getHours()) : d.getHours();
						var choose_mi = d.getMinutes() < 10 ? ("0" + d.getMinutes()) : d.getMinutes();
						var choose_s = d.getSeconds() < 10 ? ("0" + d.getSeconds()) : d.getSeconds();
						var stringYMD = choose_y + "-" + choose_mo + "-" + choose_d;
						var stringYMDHMS = choose_y + "-" + choose_mo + "-" + choose_d + " " + choose_h + ":" + choose_mi + ":" + choose_s;
						obj.value = stringYMD;
		...
		handleDefaultPage: function(jsonObj) { //缺省页面处理
			var defaultImgSrc = '',
				...
				objDiv = jsonObj.objdiv,
				_html = '',
				domObj = $("#" + $.ui.activeDiv.id + " .list-nodata");
			switch(type) {
				case 1:
			...
			_html += '<div class = "list-nodata">'
			_html += '<img src = "' + defaultImgSrc + '" alt = "图片说明" />'
			_html += '<p>' + defaultDesc + '</p>'
			_html += '</div>'
			$("#" + $.ui.activeDiv.id + "Content .list-nodata").remove();
			if(objDiv != undefined) {
				$(objDiv).empty().append(_html);
			} else {
				$("#" + $.ui.activeDiv.id + "Content").empty().append(_html);
			}
		},
					...
					case 1:
						plus.camera.getCamera().captureImage(function(filepath) {
							...
							plus.io.resolveLocalFileSystemURL(filepath, function(entry) {
								...
								common.uploadSelectedPicture(localurl, el_obj, 0, selected.index);
							});
						});
						break;
					case 2:
						plus.gallery.pick(function(e) {
							...
							common.uploadSelectedPicture(e.files[0], el_obj, 0, selected.index);
						}, function(e) {
							console.log("取消选择图片");
						}, {
							filter: "image",
		...
		uploadSelectedPicture: function(filepath, uploadDiv, fileindex, choiceIndex) {
			$.ui.showMask("图片上传中");
			...
			plus.zip.compressImage({
				...
				function(e) {
					...
					image.onload = function() {
						...
						var params = {};
						params.imgStr = base64;
						ajaxFuncs.post({
							urlname: "/api/headPhoto",
							params,
							funcs: {
								funcSuccessful: function(data) {
									//									$("#" + uploadDiv).parent().find("img").attr("src", "http://" + data.absolutePath);
									if($(uploadDiv).parent().find("input[name='photo']").length != 0) {
										$(uploadDiv).parent().find("input[name='photo']").val(data.absolutePath);
									}
									if($(uploadDiv).parent().find("input[name='headPhoto']").length != 0) {
										$(uploadDiv).parent().find("input[name='headPhoto']").val(data.absolutePath);
									}
								}
							}
						});
		...
		touchDel: function(pageName) {
			var container = $("#" + pageName + ' .container-box');
					...
					case 1:
						plus.camera.getCamera().captureImage(function(filepath) {
							...
							plus.io.resolveLocalFileSystemURL(filepath, function(entry) {
								...
								console.log(localurl);
								//								$(el_obj).attr("src", localurl);
								//								$(el_obj).parent().find('input').val(localurl);
								common.uploadSelectedPictureLoca(localurl, el_obj, 0, selected.index);
							});
						});
						break;
					case 2:
						plus.gallery.pick(function(e) {
							console.log(e.files[0]);
							//							$(el_obj).attr("src", e.files[0]);
							//							$(el_obj).parent().find('input').val(e.files[0]);
							common.uploadSelectedPictureLoca(e.files[0], el_obj, 0, selected.index);
						}, function(e) {
							console.log("取消选择图片");
						}, {
							filter: "image",
		...
		uploadSelectedPictureLoca: function(filepath, uploadDiv, fileindex, choiceIndex) {
			//			$.ui.showMask("图片上传中");
			var suffix = filepath.substring(filepath.lastIndexOf(".") + 1);
			var newname = new Date().getTime();
			var picName = newname + fileindex + "." + suffix;
			plus.zip.compressImage({
				...
				function(e) {
					...
					html += '<img src="' + e.target + '" alt="照相" />';
					html += '<input type="hidden"  placeholder="申请表图片" value="' + e.target + '" />';
					html += '</div>';
					$(uploadDiv).parent().parent().find('.savebox .saveboxList').append(html);
					$('.saveboxListItem').unbind("longTap");
					$('.saveboxListItem').bind("longTap", function(event) {
						event.stopPropagation();
						var that = this;
						$.ui.popup({
							title: "删除确认",
					...
					if($(uploadDiv).parent().parent().find('.saveboxListItem').length == 2) {
						$(uploadDiv).parent().hide();
					}
				},
				function(err) {
					$.ui.hideMask();
		...
		uploadSelectedPictureServers: function(url, callback) {
			var base64Rtrun;
			...
			image.onload = function() {
				...
				base64 = base64.slice((base64.indexOf(',') + 1));
				base64Rtrun = base64;
				callback(base64Rtrun);
				//				return base64Rtrun;
			}

		},
		getPageHistoryId: function() {
			var historyArr = $.ui.history,
				el_pageHistoryId = '';
			if(historyArr.length > 0) {
				var el_pageHistoryId = historyArr[historyArr.length - 1].target;
			}
			return el_pageHistoryId;
		},
		getPageHistoryName: function() {
			var el_pageHistoryName = (this.getPageHistoryId()).substring(1);
			return el_pageHistoryName;
		},
