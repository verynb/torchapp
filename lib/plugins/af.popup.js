	...
	var popup = (function() {
		var popup = function(containerEl, opts) {
			...
			try {
				...
				this.doneText = opts.doneText || "提交";
		...
		popup.prototype = {
			...
			suppressTitle: false,
			show: function() {
				G_POPUPSHOW = true; //ymkadun-2016/1/6
				var showPage = $.ui.activeDiv.id;
//				var hidePage = '';
				var showPopup = setInterval(function() { //xcl-2017/5/9
					var hidePage = $.ui.activeDiv.id;
					if(showPage != hidePage) {
						self.hide();
						window.clearInterval(showPopup);
					} else if(G_POPUPSHOW = false) {
						window.clearInterval(showPopup);
					}
				}, 100)
				var self = this;
				...
				var markup = "<div id='" + this.id + "' class='afPopup hidden" + this.addCssClass + "'>" +
					...
					"<div style='overflow-y:auto'>" + this.message + "</div>" +
					"<footer>" +
				...
				$($("#" + this.id).find("div").get(0)).css("max-height", height);
			...
			hide: function() {
				G_POPUPSHOW = false; //ymkadun-2016/1/6
				var self = this;
