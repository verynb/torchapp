...
(function($) {
	...
	ui.prototype = {
		...
		showLoading: true,
		loadingText: "", //loadingText: "Loading Content",
		loadContentQueue: [],
		isIntel: false,
		titlebar: "",
		navbar: "",
		...
		*/
		...
		goBack: function(delta) {
			...
			if(delta) {
				...
				var topRightMenuStr;
				var helpStr;
				if($.ui.history.length == 0) { //首页即待办事务页
					topRightMenuStr = "";
					helpStr = "";
				} else {
					topRightMenuStr = $.ui.history[$.ui.history.length - 1].topRightMenu ? $.ui.history[$.ui.history.length - 1].topRightMenu : "";
					helpStr = $.ui.history[$.ui.history.length - 1].topRightHelp ? $.ui.history[$.ui.history.length - 1].topRightHelp : "";
				}
				$("#topRightMenu ul").html(topRightMenuStr);
				$("#aside_menu .y-scroll div").html(helpStr);
				if(topRightMenuStr == "") {
					$("#header #topRightMenuBtn").hide();
				} else {
					$("#header #topRightMenuBtn").show();
				}
				$("#header #navButtonIndex").show(); //帮助入口

				var tmpEl = this.history.splice(-delta).shift(); //删除history最后一个，即当前跳转到的页面将从history里被删除
				if(tmpEl.target === "#welcome") {
					this.loadContent(G_ISADMIN ? "#adminmain" : "#main", 0, 1, tmpEl.transition);
					this.transitionType = tmpEl.transition;
					this.updateHash(G_ISADMIN ? "#adminmain" : "#main");
				} else {
					var gobackIsRefresh = {
						'refresh': true
					}
					this.loadContent(tmpEl.target + "", 0, 1, tmpEl.transition, gobackIsRefresh);
					this.transitionType = tmpEl.transition;
		...
		*/
				...
				if($panel.data("header") === "none") { // no header
					...
					modalParent.find("#modalHeader").append(
						$.create("header", {}).append(
							$.create("h1", {
								html: $panel.data("title")
							}).get(0))
						.append(
							$.create("a", {
								className: "button icon close"
							}).attr("onclick", "$.ui.hideModal()").get(0)
						)).show();
				}
				//modal footer
		...
		loadContent: function(target, newTab, back, transition, anchor) {
			...
			if($("#navbar footer a").length < 1 || $("#navbar").attr("showorhide") == "hide") {
				$("#navbar").css("display", "none");
			}
			var targetKey, panelFooter, showFooter;
			targetKey = target.indexOf("#") == -1 ? target : target.split("#")[1];
			panelFooter = $("#" + targetKey).attr("data-footer");
			showFooter = $("#navbar").attr("showorhide") ? $("#navbar").attr("showorhide") : ($("#producereview_2_" + targetKey).attr("data-showfooter") == "false" ? "hide" : "show");
			if(!panelFooter) { //判断target页面是否有footer
				$("#navbar").css("display", "none");
			} else {
				if(webContent[targetKey]) { //判断其是否有新建页面
					var targetPage = webContent[targetKey].hasOwnProperty("newaction") ? webContent[targetKey].newaction : targetKey;
					var targetDir = webContent[targetKey].hasOwnProperty("basedatadir") ? webContent[targetKey].basedatadir : "base";
					$("#navbar").css("display", "block");
					$("#" + targetKey + "footer .newaction").attr("data-tkey", targetPage);
					$("#" + targetKey + "footer .newaction").attr("data-dir", targetDir);
					$("#" + targetKey + "footer .newaction").attr("href", "javascript:void(0);");
					$("#" + targetKey + "footer .newaction").attr("onclick", "loadPage(this,false)");
				} else {
					if(showFooter == "hide") {
						$("#navbar").css("display", "none");
						$("#navbar").removeAttr("showorhide");
					} else {
						$("#navbar").css("display", "block");
					}
				}
			...
			/*若是返回操作，则从history里抓取右上角菜单和帮助；若不是，则从json文件重新读取菜单和帮助*/
			if(!back) { //不是返回
				//获取新菜单，帮助
				var tkey;
				if(target.indexOf("#") != -1) { //找到#
					tkey = target.substring(1, target.length);
				} else { //未找到#
					tkey = target;
				}
				var panelDir = $("#" + tkey).attr("data-dir");
				common.resetTopRightMenu({
					dir: panelDir,
					tkey: tkey
				}); //重新抓取右上角菜单
			}
			//          $.ui.scrollToTop(target);
			if($.ui.history.length > 0) { //即将去的页面和当前历史记录最后一条数据若相同，则将最后一条历史数据清除
				if("#" + target == $.ui.history[$.ui.history.length - 1].target) {
					//新增于2016-3-17ym
					if(!newTab && back) { //2016-3-17   为了满足$.ui.loadContent(param.parentpage,false,true,'slide');主要是loadContent第三个为true和第二个参数为false的情况
						//抓取历史菜单放入右上角菜单和帮助区
						var topRightMenuStr;
						var helpStr;
						if($.ui.history.length == 0) { //首页即待办事务页
					...
					$.ui.history.splice($.ui.history.length - 1, 1); //删除数组最后一个元素
				}
			}
		...
		loadDiv: function(target, newTab, back, transition) {
			...
			if(!what) {
				$(document).trigger("missingpanel", null, {
			...
			if(newTab) {
				...
				var notPushHistory = $("#" + this.firstDiv.id).attr("data-nopushhistory");
				if(notPushHistory != "1") {
					this.pushHistory("#" + this.firstDiv.id, what.id, transition, hashLink);
				}
				...
				var notPushHistory = $(previousTarget).attr("data-nopushhistory");
				if(notPushHistory != "1") {
					this.pushHistory(previousTarget, what.id, transition, hashLink); //装入页面
					//在获取新菜单之前，先将上个页面菜单帮助等装入history
					//将右上角菜单和帮助内容都存入history
					var topRightMenuStr = $("#topRightMenu ul").html(); //即将离开页面的右上角菜单
					var helpStr = $("#aside_menu .y-scroll div").html(); //即将离开页面的帮助内容
					if(topRightMenuStr != "") {
						$.ui.history[$.ui.history.length - 1].topRightMenu = topRightMenuStr;
					}
					if(helpStr != "") {
						$.ui.history[$.ui.history.length - 1].topRightHelp = helpStr;
					}
				}
			}
		...
		loadContentData: function(what, newTab, back) {
			...
			//if($.query("#navbar #defaultNav").length>0) $.query("#navbar").css("display","none");
			if(back) {
				...
				$.query("#headerfunction").css("display", "none");
				for(var i = 0; i < webContent.searchHistory.length; i++) {
					if(webContent.searchHistory[i] == what.id) {
						$.query("#headerfunction").css("display", "block");
						webContent.searchHistory.pop();
						break;
					}
				}
				if(what.id == "main") {
					$.query("#headerfunction #titlenew").css("display", "none");
				}
			} else if(this.activeDiv.getAttribute("data-title")) {
				this.setBackButtonText(this.activeDiv.getAttribute("data-title"));
				//$.query("#"+what.id+" .afScrollPanel").append("<div style='height: 50px;'></div>");
			} else {
